      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Track stock levels, minimum reorder alerts, warehouse locations & stock logs.
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-900/40 p-4 rounded-xl border border-slate-800">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-semibold border flex items-center justify-center gap-2 transition-all ${
                lowStockOnly
                            ...
                            <div className="flex items-center gap-2">
                              <span
                                className={`font-bold text-sm ${
                                  isLowStock ? 'text-amber-400 font-extrabold' : 'text-emerald-400'
                                }`}
                              >
                              ...
                              {canManageWarehouse && (
                                <>
                                  <button
                                    ...
                                    className="px-2.5 py-1.5 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/30 rounded-lg text-[11px] font-semibold transition-colors flex items-center gap-1"
                                    title="Stock IN / Stock OUT adjustment"
                                  >
                  ...
                  stockLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="p-4 font-semibold text-slate-200">
                        ...
                        <div className="text-[10px] font-mono text-slate-500">{log.sku}</div>
                      </td>
                      <td className="p-4">
                        <span
                      ...
                      <td className="p-4 text-slate-400">{log.createdByName}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl">
            ...
            <form onSubmit={handleSubmitProduct} className="space-y-4 text-xs">
              ...
              {!editingProduct && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Initial Opening Stock</label>
                    <input
                      type="number"
                      min="0"
                      required
                  ...
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Min Stock Alert Qty</label>
                    <input
                      type="number"
                      min="0"
                      required
              ...
              <div className="pt-3 border-t border-slate-800 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl shadow-lg shadow-emerald-600/20"
                >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                ...
                <h3 className="text-base font-bold text-white mt-1">{adjustingProduct.name}</h3>
                <p className="text-xs text-slate-400">Current Stock: {adjustingProduct.currentStock} units</p>
              </div>
            ...
            <form onSubmit={handleSubmitAdjustment} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 font-medium mb-1">Movement Type</label>
                  <select
                    value={adjustType}
              ...
              <div>
                <label className="block text-slate-400 font-medium mb-1">Reason / Reference *</label>
                <input
                  type="text"
                  required
                  value={adjustReason}
